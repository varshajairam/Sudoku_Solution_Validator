...
void *checkRow(void* rowData) {
    ...
    thread_result[9 + row/3] = 1; // Maps result of each row validation to indices 9-11 of the validity array
    pthread_exit(nullptr);
}
...
void *checkColumn(void* colData) {
    ...
    thread_result[12 + col/3] = 1; // Maps result of each column validation to indices 12-14 of the validity array
    pthread_exit(nullptr);
}
...
int main() {
    ...
    for(int i=0; i<9; i++){
        for(int j=0; j<9; j++){
            if(i%3 == 0 && j == 0){
                ...
                rowData->row = i;
                rowData->column = j;
                pthread_create(&threads[threadId++], nullptr, checkRow, rowData); // 3 row threads
            }
            if(i == 0 && j%3 == 0){
                auto *columnData = new inputParams[sizeof(inputParams)];
                columnData->row = i;
                columnData->column = j;
                pthread_create(&threads[threadId++], nullptr, checkColumn, columnData); // 3 column threads
            }
            if(i%3 == 0 && j%3 == 0){
    ...
    for (int i : thread_result) {
        if (i == 0) {
            ...
            cout << endl << "Total time using 15 threads: " << elapsed_time_threads.count() << " seconds" << endl;            
            return 0;
        }
    }
